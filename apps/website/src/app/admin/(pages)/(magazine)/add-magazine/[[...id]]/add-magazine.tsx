...
import {  getAllCategorys, getAllUsers, getMagazineById, submitMagazine, uniqueMagazine } from "@/src/services/admin/services";
...
export default function AddMagazineForm() {
  ...
  const { options: categoryOptions } = useLabeledOptions({
    ...
    const mapUserLabel = (item: { full_name: string; }) => item.full_name;
  const mapUserValue = (item: { id: string | number; }) => Number(item.id);
  ...
  const { options: userOptions } = useLabeledOptions({
    ...
    mapValue: (item) => Number(mapUserValue(item)), 
    onError: (err) => console.error(err),
  });
  ...
  const onSubmit = async (data: MagazineFormData) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate.push("/admin/magazine-list");
      }, 2000);
    } catch (err: any) {
